...
The distribution is left skewed, the mean is less than the median. The score for most instructor are high (around 4)

2.  Visualize and describe the relationship between `score` and `bty_avg` using `geom_point()` to represent the data. 

```{r scatterplot}
ggplot(data = evals,
       aes(x = score, y = bty_avg))+
  geom_point()
...
"Jitter" means irregular. The initial one is at a unrealistic level of exact and concise which the data value isn't as likely to happen in real life.
...
The score the instructor receive rise on an average of (slope value) as bty_avg rises by 1.
...
When the bty_avg score of the instructor is 0.066 (really ugly), the score they receive is 3.88 approximately. This doesn't quite make sense in real life because it's too ridiculous and the scoring guideline of "beauty" isn't clear on this set of data.
...
A value of R^2 really close to zero, indicating only a small portion of data can be count for the regression line being calculated.
...
ggplot(data = score_bty_aug, aes(x = .fitted, y = .resid)) + 
